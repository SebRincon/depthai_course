...
import depthai_camera

dpg.create_context()
texture_data = depthai_camera.disparityImage()
...
with dpg.window(tag="Top Window", width=1200, height=30, pos=(0, 0)):
    ...
    dpg.add_progress_bar(show=True, overlay='UTC', width=200,
                         height=30, default_value=0.5, pos=(0, 30))
    dpg.add_progress_bar(show=True, overlay='Bin Capacity',
                         width=200, height=30, default_value=0.5, pos=(230, 30))
    dpg.add_progress_bar(show=True, overlay='Battery',
                         width=200, height=30, default_value=0.5, pos=(460, 30))
    dpg.add_progress_bar(show=True, overlay='Rover Temp',
                         width=200, height=30, default_value=0.5, pos=(690, 30))
...
with dpg.window(tag='Bottom Window', width=1200, height=300, pos=(0, 500), label='Surface Operations'):
    dpg.bind_font(default_font)
    with dpg.group(horizontal=True):
        dpg.add_listbox(items=["Start Up", "Calibration", "Pathing",
                               "Excavation", "Deposition"], width=200, num_items=6)
            ...
            with dpg.plot(no_title=True):
                ...
                with dpg.plot_axis(dpg.mvYAxis, label="Time", no_tick_labels=True):
                    dpg.add_line_series(sindatax, sindatay,
                                        label="0.5 + 0.5 * sin(x)")
            with dpg.plot(no_title=True):
                dpg.add_plot_axis(dpg.mvXAxis, label="W/h",
                                  no_tick_labels=True)
                with dpg.plot_axis(dpg.mvYAxis, label="Time", no_tick_labels=True):
                    dpg.add_line_series(sindatax, sindatay,
                                        label="0.5 + 0.5 * sin(x)")
...
while dpg.is_dearpygui_running():
    ...
    dpg.set_value("texture_tag", texture_data["texture"])
